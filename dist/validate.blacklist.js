...
function validateBlacklist({ label, setting, value }) {
    ...
    if (typeof value === "string" && typeof setting === "string") {
        ...
        if (typeof setting.exact === "string") {
            ...
            if (value.toLowerCase() === setting.loose.toLowerCase())
                term = setting.loose;
        }
            ...
            term = setting.loose.find((set) => {
                return value.toLowerCase() === set.toLowerCase();
            });
        }
    }
        ...
        if (typeof setting.exact === "string") {
            ...
            // if (value.some((v) => v.toLowerCase() === setting.loose.toLowerCase()))
            //   term = setting.loose;
        }
        else if (Array.isArray(setting.exact)) {
            ...
            //   return value.some((v) => v.toLowerCase() === set.toLowerCase());
            // });
        }
    }
    if (term)
