...
function validateBlacklist({ label, setting, value }: Validation) {
  ...
  if (typeof value === "string" && typeof setting === "string") {
    if (value.trim() === setting) term = setting;
    ...
    if (typeof setting.exact === "string") {
      if (value === setting.exact) term = setting.exact;
    ...
    if (typeof setting.exact === "string") {
      if (value.some((v) => v === setting.exact)) term = setting.exact;
    } else if (typeof setting.loose === "string") {
      if (
        value.some((v) => v.toLowerCase().includes(setting.loose.toLowerCase()))
      )
        term = setting.loose;
    } else if (Array.isArray(setting.exact)) {
      term = setting.exact.find((set: string) => value.some((v) => v === set));
    } else if (Array.isArray(setting.loose)) {
      term = setting.loose.find((set: string) => {
  ...
  if (term) throw new Error(`${label}: "${term}" is a reserved term.`);
}
